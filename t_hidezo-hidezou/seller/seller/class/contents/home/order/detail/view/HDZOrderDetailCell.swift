...
protocol OrderDetailItemUpdateDelegate: NSObjectProtocol {
...
class HDZOrderDetailCell: UITableViewCell {
    ...
    private weak var delegate: OrderDetailItemUpdateDelegate? = nil
...
extension HDZOrderDetailCell {
    ...
    internal static func priceValue(price: String, order_num: String, attr_flg: AttrFlg) -> Int {
        
        let prices: [String] = price.componentsSeparatedByString(",") // .components(separatedBy: ",")
        if prices.count < 1 {
            return 0
        }
...
extension HDZOrderDetailCell: HDZOrderScaleViewControllerDelegate {
    ...
    func didSelectRow(numScale: String, row: Int) {
        ...
        self.delegate?.didSelectedItemUpdate(self.orderDetailItem, order_num: numScale)
    }
}
