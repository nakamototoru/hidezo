...
extension AlamofireUtils {
    ...
    internal class func request<T: Unboxable, U: WrapCustomizable, V: Unboxable>(method: Alamofire.Method, _ URLString: URLStringConvertible, structParameters: U, encoding: ParameterEncoding = ParameterEncoding.URL, headers: [String : String]? = nil, completionBlock: (unboxable: T?) -> Void, errorBlock: (error: ErrorType?, unboxable: V?) -> Void) -> Alamofire.Request? {
...
extension AlamofireUtils {
    ...
    internal class func upload<T: Unboxable, U: WrapCustomizable>(method: Alamofire.Method, _ URLString: URLStringConvertible, structParameters: U, headers:[String : String]? = nil, bodyPart: [AlamofireBodyPartItem], completionBlock: (unboxable: T?) -> Void, errorBlock: (error: ErrorType?, result: T?) -> Void) {
        ...
        let completionHandler: ((Manager.MultipartFormDataEncodingResult) -> Void) = { (response: Manager.MultipartFormDataEncodingResult) in
            switch response {
            case Manager.MultipartFormDataEncodingResult.Failure(let error):
                errorBlock(error: error, result: nil)
                break
            case Manager.MultipartFormDataEncodingResult.Success(let request, _, _):
