...
final class GalleryPresentTransition: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if let imageView = imageView, resizeToAspectFit {
            ...
            UIView.animate(withDuration: 0.25, animations: {
                layoutAspectFit(view: self.displacedView, image: imageView.image!)
                }, completion: { [weak self] finished in
                    self?.animateDisplacedView(transitionContext: transitionContext, onComplete: {
                        self?.displacedView.frame = originalFrame
                    })
                })
        } else {
            self.animateDisplacedView(transitionContext: transitionContext)
        }
    }
    ...
    private func animateDisplacedView(transitionContext: UIViewControllerContextTransitioning, onComplete: ((Void) -> Void)? = nil) {
        ...
        let screenshot = screenshotFromView(view: displacedView)
