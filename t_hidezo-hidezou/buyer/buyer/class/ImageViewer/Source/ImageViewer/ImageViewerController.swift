...
public final class ImageViewerController: UIViewController, UIScrollViewDelegate, UIViewControllerTransitioningDelegate {
    ...
    private func configureCloseButton() {
        ...
        closeButton.addTarget(self, action: #selector(ImageViewerController.close), for: .touchUpInside)
    }
    
    private func configureGestureRecognizers() {
        
        doubleTapRecognizer.addTarget(self, action: #selector(ImageViewerController.scrollViewDidDoubleTap))
        doubleTapRecognizer.numberOfTapsRequired = 2
        scrollView.addGestureRecognizer(doubleTapRecognizer)
        
        panGestureRecognizer.addTarget(self, action: #selector(ImageViewerController.scrollViewDidPan))
    ...
    private func configureImageView() {
        ...
        imageView.image = screenshotFromView(view: displacedView)
    }
...
//	public override func viewwill
    ...
    func showAnimation(duration: TimeInterval, completion: ((Bool) -> Void)?) {
        ...
        UIView.animate(withDuration: duration, animations: {
            ...
            self.imageView.bounds = CGRect(origin: CGPoint.zero, size: aspectFitSize)
            self.imageView.center = rotationAdjustedCenter(view: self.view)
            self.scrollView.contentSize = self.imageView.bounds.size
            ...
            if finished {
                if isPortraitOnly() {
                    ...
                    NotificationCenter.default.addObserver(self, selector: #selector(ImageViewerController.rotate), name: Notification.Name.UIDeviceOrientationDidChange, object: nil)
                }
    ...
    @objc private func scrollViewDidPan(recognizer: UIPanGestureRecognizer) {
        ...
        case .ended:
            handleSwipeToDismissEnded(verticalVelocity: verticalVelocity, lastTouchPoint: lastTouchPoint, targetOffset: targetOffsetToReachEdge)
    ...
    func handleSwipeToDismissEnded(verticalVelocity: CGFloat, lastTouchPoint: CGPoint, targetOffset: CGFloat) {
        ...
        case thresholdVelocity ... cutOffVelocity:
            ...
            swipeToDismissTransition.setParameters(verticalTouchPoint: touchPoint, targetOffset: offset, verticalVelocity: verticalVelocity)
            presentingViewController?.dismiss(animated: true, completion: nil)
