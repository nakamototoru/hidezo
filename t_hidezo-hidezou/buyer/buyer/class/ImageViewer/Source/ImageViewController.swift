...
final class ImageViewController: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate, UIViewControllerTransitioningDelegate {
    ...
    init(imageProvider: ImageProvider, configuration: GalleryConfiguration, imageCount: Int, displacedView: UIView, startIndex: Int,  imageIndex: Int, showDisplacedImage: Bool, fadeInHandler: ImageFadeInHandler?, delegate: ImageViewControllerDelegate?) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(ImageViewController.adjustImageViewForRotation),
    ...
    func configureImageView() {
        ...
        if showDisplacedImage {
            updateImageAndContentSize(image: screenshotFromView(view: displacedView))
        }
        
        self.fetchImage(atIndex: self.index)  { [weak self] image in
			
			DispatchQueue.main.async {
				if let fullSizedImage = image {
					self?.updateImageAndContentSize(image: fullSizedImage)
				}
			}
...
//            }
    ...
    func fetchImage(atIndex: Int, completion: @escaping (UIImage?) -> Void) {
...
//        }
    ...
    func configureScrollView() {
        ...
        scrollView.contentInset = UIEdgeInsets.zero // Zero
        scrollView.contentOffset = CGPoint.zero // Zero
    ...
    func configureGestureRecognizers() {
        ...
        singleTapRecognizer.addTarget(self, action: #selector(ImageViewController.scrollViewDidSingleTap))
        singleTapRecognizer.numberOfTapsRequired = 1
        scrollView.addGestureRecognizer(singleTapRecognizer)
        
        doubleTapRecognizer.addTarget(self, action: #selector(ImageViewController.scrollViewDidDoubleTap))
        doubleTapRecognizer.numberOfTapsRequired = 2
        scrollView.addGestureRecognizer(doubleTapRecognizer)
        
        singleTapRecognizer.require(toFail: doubleTapRecognizer)
        
        panGestureRecognizer.addTarget(self, action: #selector(ImageViewController.scrollViewDidSwipeToDismiss))
        ...
        if self.needToAnimateImage() {
            UIView.transition(with: self.scrollView, duration: 0.5, options: UIViewAnimationOptions.transitionCrossDissolve, animations: { () -> Void in
                self.imageView.image = image
                    ...
                    self.fadeInHandler?.addPresentedImageIndex(index: self.index)
            })
        } else {
            self.imageView.image = image
            self.fadeInHandler?.addPresentedImageIndex(index: self.index)
    ...
    private func needToAnimateImage() -> Bool {
        ...
        return !fadeInHandler.wasPresented(index: self.index) && self.index != self.startIndex
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.delegate?.imageViewControllerDidAppear(controller: self)
    }
...
//    }
    ...
    func scrollViewDidSingleTap(recognizer: UITapGestureRecognizer) {
        ...
        self.delegate?.imageViewControllerDidSingleTap(controller: self)
    }
    ...
    func scrollViewDidSwipeToDismiss(recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            self.handleSwipeToDismissInProgress(swipeOrientation: swipingToDismissInProgress, forTouchPoint: currentTouchPoint)
            
        case .ended:
            self.handleSwipeToDismissEnded(swipeOrientation: swipingToDismissInProgress, finalVelocity: currentVelocity, finalTouchPoint: currentTouchPoint)
    ...
    func handleSwipeToDismissEnded(swipeOrientation: SwipeToDismiss, finalVelocity velocity: CGPoint, finalTouchPoint touchPoint: CGPoint) {
        ...
        case (.Vertical, _) where velocity.y < -thresholdVelocity: /// All images VERTICAL UP direction
            ...
            swipeToDismissTransition?.finishInteractiveTransition(swipeDirection: swipeOrientation, touchPoint: touchPoint.y, targetOffset: (view.bounds.height / 2) + (imageView.bounds.height / 2), escapeVelocity: velocity.y) {  [weak self] in
        ...
        case (.Horizontal, 0) where thresholdVelocity < velocity.x: /// First image HORIZONTAL RIGHT direction
            ...
            swipeToDismissTransition?.finishInteractiveTransition(swipeDirection: swipeOrientation, touchPoint: touchPoint.x, targetOffset: -(view.bounds.width / 2) - (imageView.bounds.width / 2), escapeVelocity: velocity.x) {  [weak self] in
                self?.swipingToDismiss = nil
                self?.applicationWindow?.windowLevel = UIWindowLevelNormal
                parentViewController?.swipedToDismissCompletion?()
                presentingViewController?.dismiss(animated: false, completion: nil)
    ...
    func closeAnimation(duration: TimeInterval, completion: ((Bool) -> Void)?) {
...
//            delegate.imageViewController(controller: self, didSwipeToDismissWithDistanceToEdge: percentDistance)
//        }
//    }
